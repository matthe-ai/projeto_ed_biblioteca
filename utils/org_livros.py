...
class Livros_arvore:
    ...
    def _inserir(self, no, livro):
        ...
        # EE
        if (balanceamento > 1 and livro.titulo.lower() < no.esq.livro.titulo.lower()):
            return self.rotacao_direita(no)

        # DD
        if (balanceamento < -1 and livro.titulo.lower() > no.dir.livro.titulo.lower()):
    ...
    def buscar_titulo(self, titulo):
        return self._buscar_titulo(self.raiz, titulo.lower())
    ...
    def _buscar_titulo(self, no, titulo):
        ...
        if titulo < atual:
            return self._buscar_titulo(no.esq, titulo)

        return self._buscar_titulo(no.dir, titulo)
    ...
    def remover(self, titulo:str):
        self.raiz = self._remover(self.raiz, titulo.lower())
    ...
    def _remover(self, no, titulo):
        ...
        if titulo < atual:
            ...
            no.esq = self._remover(no.esq, titulo)

        elif titulo > atual:

            no.dir = self._remover(no.dir, titulo)
            ...
            no.dir = self._remover(no.dir, sucessor.livro.titulo.lower())
        ...
        # EE
        if (balanceamento > 1 and self.fator_balanceamento(no.esq) >= 0):
            return self.rotacao_direita(no)
        # ED
        ...
        # DD
        if (balanceamento < -1 and self.fator_balanceamento(no.dir) <= 0):
            return self.rotacao_esquerda(no)
